...
rep_matches = [len([m for m in re.finditer('(R|r)epublican', html.text)])
               for html in sites]
dem_matches = [len([m for m in re.finditer('(D|d)emocrat', html.text)])
               for html in sites]
trump_matches = [len([m for m in re.finditer('Trump', html.text)])
                 for html in sites]

df = pd.DataFrame({'Site': list(urls.keys()), 'Republican': rep_matches,
                   'Democrat': dem_matches, 'Trump': trump_matches})
df
...
sixties = [movie.title for movie in movies if movie.year <
           1960 and movie.year > 1954]
...
statuses = twitter.search(
    q='impeach', tweet_mode='extended', count=5000)['statuses']
...
d = {'trump': trump, 'schiff': schiff, 'bolton': bolton,
     'mcconnell': mcconnell, 'romney': romney}
...
stream = MyStreamer(CONSUMER_KEY, CONSUMER_SECRET_KEY,
