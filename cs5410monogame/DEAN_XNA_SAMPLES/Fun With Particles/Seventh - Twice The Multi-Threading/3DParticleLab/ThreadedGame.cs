...
namespace ParticleLab
{
	...
	/// <summary>
	/// This program demonstrates how to use multi-threading in an XNA program.  The computation
	/// for the particle updates are performed in a second thread, reducing the delay between
	/// the ability to draw each frame.
	/// </summary>
	public class ThreadedGame : Microsoft.Xna.Framework.Game
	{
		...
		protected override void Initialize()
		{
			...
			//
			// Create a particle computation thread
			m_UpdateParticles1 = new AutoResetEvent(false);
			m_UpdateParticles2 = new AutoResetEvent(false);
			m_ComputeThread1 = new Thread(new ThreadStart(ParticleThread1));
			m_ComputeThread2 = new Thread(new ThreadStart(ParticleThread2));

			m_ComputeThread1.Start();
			m_ComputeThread2.Start();

			base.Initialize();
		}

		#region Particle Threading

		/// <summary>
		/// Thread for computing the first group of particles
		/// </summary>
		private void ParticleThread1()
		{
#if XBOX360
			m_ComputeThread1.SetProcessorAffinity(new int[] {4});
#endif
			try
			{
				while (true)
				{
					m_UpdateParticles1.WaitOne();
					lock (m_LockParticles1)
					{
						//
			...
			{
				//
				// This just means it is okay to quit.
			}
		}

		/// <summary>
		/// Thread for computing the second group of particles
		/// </summary>
		private void ParticleThread2()
		{
#if XBOX360
			m_ComputeThread2.SetProcessorAffinity(new int[] {5});
#endif
			try
			{
				while (true)
				{
					m_UpdateParticles2.WaitOne();
					lock (m_LockParticles2)
					{
		...
		private Thread m_ComputeThread1;
		private Thread m_ComputeThread2;
		private AutoResetEvent m_UpdateParticles1;
		private AutoResetEvent m_UpdateParticles2;
		private object m_LockParticles1 = new object();
		private object m_LockParticles2 = new object();
		...
		protected override void Update(GameTime gameTime)
		{
			...
			{
				m_ComputeThread1.Abort();
				m_ComputeThread2.Abort();
				this.Exit();
			}
#endif
		...
		protected override void Draw(GameTime gameTime)
		{
			...
			//
			// Let the compute thread know it is okay to compute
			m_UpdateParticles1.Set();
