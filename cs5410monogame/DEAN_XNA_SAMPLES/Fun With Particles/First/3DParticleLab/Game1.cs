...
namespace ParticleLab
{
	...
	public class MagicNumbersAbound : Microsoft.Xna.Framework.Game
	{
		...
		protected override void Draw(GameTime gameTime)
		{
            ...
            graphics.GraphicsDevice.DepthStencilState = m_depthStencilState;
			...
			{
				DrawModel(p.Model, p.Texture, p.Rotation, p.Center,p.Scale);
			}
		...
		private void DrawModel(Model DrawMe,Texture2D Texture, Vector3 Rotation, Vector3 Position,float Scale)
		{
			//
			// Perform the bone transformations
			Matrix[] BoneTransforms = new Matrix[DrawMe.Bones.Count];
			DrawMe.CopyAbsoluteBoneTransformsTo(BoneTransforms);

			//
			// Draw the model. A model can have multiple meshes, so loop.
			foreach (ModelMesh Mesh in DrawMe.Meshes)
			{
				// This is where the mesh orientation is set, as well as our 
