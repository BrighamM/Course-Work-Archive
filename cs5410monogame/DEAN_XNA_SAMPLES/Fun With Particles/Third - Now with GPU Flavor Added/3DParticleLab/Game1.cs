...
namespace ParticleLab
{
	...
	public class MagicNumbersAbound : Microsoft.Xna.Framework.Game
	{
		...
		protected override void Draw(GameTime gameTime)
		{
			...
			m_fxParticles[(int)this.CurrentEffect].Parameters["View"].SetValue(m_mView);
			m_fxParticles[(int)this.CurrentEffect].Parameters["Projection"].SetValue(m_mProjection);

			//
			// All models are the same
			ModelMeshPart MeshPart = m_Models[0].Meshes[0].MeshParts[0];

			graphics.GraphicsDevice.Vertices[0].SetSource(m_Models[0].Meshes[0].VertexBuffer, MeshPart.StreamOffset, MeshPart.VertexStride);
			graphics.GraphicsDevice.VertexDeclaration = MeshPart.VertexDeclaration;
			graphics.GraphicsDevice.Indices = m_Models[0].Meshes[0].IndexBuffer;

			m_fxParticles[(int)this.CurrentEffect].CurrentTechnique = m_fxParticles[(int)this.CurrentEffect].Techniques[0];

			m_fxParticles[(int)this.CurrentEffect].Begin();
			m_fxParticles[(int)this.CurrentEffect].CurrentTechnique.Passes[0].Begin();

			//
			// Draw all our particles
			foreach (Particle p in this.Particles)
			{
				m_fxParticles[(int)this.CurrentEffect].Parameters["Texture"].SetValue(p.Texture);
				m_fxParticles[(int)this.CurrentEffect].Parameters["Translation"].SetValue(p.Center);
				m_fxParticles[(int)this.CurrentEffect].Parameters["mRotation"].SetValue(Matrix.CreateRotationX(p.Rotation.X) * Matrix.CreateRotationY(p.Rotation.Y) * Matrix.CreateRotationZ(p.Rotation.Z));
				m_fxParticles[(int)this.CurrentEffect].Parameters["Scale"].SetValue(p.Scale);
				m_fxParticles[(int)this.CurrentEffect].CommitChanges();
			...
			m_fxParticles[(int)this.CurrentEffect].CurrentTechnique.Passes[0].End();
			m_fxParticles[(int)this.CurrentEffect].End();
