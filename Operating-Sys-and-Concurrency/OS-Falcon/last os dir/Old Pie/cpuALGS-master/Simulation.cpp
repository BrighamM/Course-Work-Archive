...
namespace cs3100
{
  ...
  void Simulation::scheduleIo(int job)
  {
    ...
    queue.push(Event([this, job, finishTime]() { ioDone(job, finishTime); },curTime + finishTime));
  }
  ...
  float Simulation::rawLatency(int id)
  {
		return  (jobs[id].getFinalCompletionTime()) - jobs[id].creationTime;   
  }
  ...
  {
		float cpuDurations = 0.0;
		std::vector<Task> cpuTasks;
		for(unsigned int i = 0;i<jobs.size();i++)
		{
			for(unsigned int j = 0;j < jobs[i].tasks.size();j++)
			{
				if(jobs[i].tasks[j].type == Task::CPU)
				{
					cpuTasks.push_back(jobs[i].tasks[j]);						
				}
			}
		}
		for (auto&& e:cpuTasks)
		{
			cpuDurations += e.duration;
		}
    return cpuDurations/(curTime*parameters.cpus);
  }
