...
{
	char* c = new char[toConvert.size()];
...
std::vector<char*> stringToCSVect(const std::string &userInput)
{
...
int main()
{
	int p[2];
	pipe(p);
	std::cout << p[0] << " " << p[1] << " " << STDOUT_FILENO << " " << STDIN_FILENO << std::endl;
	auto pid = fork();
	if(pid == 0)
	{
...
int main()
{
	...
	while(1)
	{
		...
		std::vector<char*> args = stringToCSVect(userInput);
