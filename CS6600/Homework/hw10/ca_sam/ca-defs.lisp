...
;; I am probably doing this wrong, but I am thinking of bought as a past tense ingest sort of act.
;; I also worked with Philip on this to work out the probable syntax.
(define-ca-word
    bought
    (concept ?act (atrans :time (past) :actor (customer) :object (thing)))
    (request (test (before ?act ?actor (animate)))
           (actions (modify ?act :actor ?actor)))
    (request (test (after ?act ?thing (thing)))
           (actions (modify ?act :object ?thing)))
    )
